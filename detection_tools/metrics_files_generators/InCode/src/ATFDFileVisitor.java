...
class ATFDClassVisitor extends ASTVisitor {
	...
	public boolean visit(MethodDeclaration node) {
		...
		// Get superclasses of the embedding class
		if (superclasses == null) {
			superclasses = new ArrayList<String>();
			if (node.resolveBinding() != null) {
				ITypeBinding superclass = node.resolveBinding().getDeclaringClass();
				while(superclass != null) {
					if (superclass.isNested()) {
		...
		ATFDMethodVisitor visitor = new ATFDMethodVisitor(superclasses);
...
class ATFDMethodVisitor extends ASTVisitor {
	...
	public ATFDMethodVisitor(List<String> superclasses) {
	...
	public boolean visit(SimpleName node) {
		...
		if (superclasses.contains(declaringClass)) {
			declaringClass = superclasses.get(0);
		}
