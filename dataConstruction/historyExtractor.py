...
class HistoryExtractor(object):
		...
		for directory in self.mainDirectorys:
			for path,dirs,files in os.walk('./' + directory):
				for f in fnmatch.filter(files,'*.java'):
					try:
						className = self.__getClassName(os.path.join(path,f))
					except:
						print('yo')
					...
					if granularity == "M":
						...
						for methodName in methods:
							normalizedMethodName = self.__normalizeMethodName(methodName)
							F.write(className + '.' + normalizedMethodName + '\n')
	...
	def __getMethodeChange(self, SHA, date, filePath, changeType):
		...
		if changeType == "A":
			...
			for method in methods:
				change = self.__normalizeMethodName(method) + ";" + "ADDED"
		...
		if changeType == "D":
			...
			for method in methods:
				change = self.__normalizeMethodName(method) + ";" + "DELETED"
				changes.append(change)
		...
		if changeType == "M":
			...
			for line in diffs:
				...
				if ct is not None:
					...
					change = self.__normalizeMethodName(method) + ";" + ct
					changes.append(change)
		...
		for pc, klass in tree.filter(javalang.tree.ClassDeclaration):
			if klass.name != className:
				for p, method in klass.filter(javalang.tree.MethodDeclaration):
					...
					methodToInnerClassDictionnary[method] = klass.name
			
		methods = []
		for p, method in tree.filter(javalang.tree.MethodDeclaration):
			if method in InnerClassMethods:
				name = methodToInnerClassDictionnary[method] + '.' + method.name
			else:
				name = method.name
	...
	def __normalizeMethodName(self, methodName):
